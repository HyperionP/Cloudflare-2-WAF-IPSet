...
def update_ip_set(ip_set_name, ip_set_id, ip_addresses):
    ...
    try:
        # Create AWS WAFv2 client in us-east-1 region, us-east0-1 is default region for CLOUDFRONT in US
        ...
        response = wafv2_client.get_ip_set(
            Name=ip_set_name,
            Id=ip_set_id,
            Scope='CLOUDFRONT'  # Use CLOUDFRONT scope for Cloudflare IP set
        )
...
def lambda_handler(event, context):
    ...
    ipv4_set_name = os.environ.get('IP_SET_NAME')
    ipv4_set_id = os.environ.get('IP_SET_ID')
